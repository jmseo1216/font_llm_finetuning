...
POLYLINE_RE = re.compile(r"([MmLlZz])|(-?\d+)")


def clean_generated_polyline(text: str) -> str:
    """
    모델 출력에서 skeleton polyline 뒤의 M/L/Z + integer token만 추출합니다.
    """
...
def integer_polyline_to_svg_path(polyline: str, scale: int = 10) -> str:
    """
    M 120 105 L 142 137
    ->
...
def generate_one_svg(
    ...
    # 4. path만 추출
    # =====================================================
    gen_polyline = clean_generated_polyline(out_text)
    gen_path_d = integer_polyline_to_svg_path(
        gen_polyline,
        scale=10,
    )
