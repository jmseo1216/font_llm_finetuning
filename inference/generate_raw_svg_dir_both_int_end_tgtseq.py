...
# target point sequence용:
# skeleton points: PATH 170 273 291 273 PATH 213 293 367 389 END
POINT_SEQ_RE = re.compile(r"(PATH|END)|(-?\d+)")
...
def clean_generated_point_sequence(text: str) -> str:
    """
    모델 출력에서 'skeleton points:' 뒤의 PATH / integer / END만 추출합니다.
    END가 나오면 그 이후는 버립니다.

    예:
    <pad> skeleton points: PATH 170 273 291 273 PATH 213 293 367 389 END</s>

    ->
    PATH 170 273 291 273 PATH 213 293 367 389
    """
    ...
    for m in POINT_SEQ_RE.finditer(text):
        marker, num = m.groups()
...
def point_sequence_to_svg_path(
    point_seq: str,
    scale: int = 10,
) -> str:
    """
    point sequence를 실제 SVG path로 복원합니다.

    입력:
    PATH 170 273 291 273 349 261 PATH 213 293 367 389

    출력:
    M 17.0 27.3 L 29.1 27.3 L 34.9 26.1 M 21.3 29.3 L 36.7 38.9

    규칙:
    - PATH 다음 첫 좌표쌍은 M
    - 같은 PATH 안의 이후 좌표쌍은 L
    - 다음 PATH가 나오면 새 M 시작
    """
    toks = point_seq.split()

    out = []
    current_nums = []
    in_path = False
    first_point_in_path = True
    ...
    while i < len(toks):
        ...
        # x, y 한 쌍이 모이면 SVG command로 변환
        if len(current_nums) == 2:
            x = current_nums[0] / scale
            y = current_nums[1] / scale

            if first_point_in_path:
                out.extend([
                    "M",
                    f"{x:.1f}",
                    f"{y:.1f}",
                ])
                first_point_in_path = False
            else:
                out.extend([
                    "L",
                    f"{x:.1f}",
...
def generate_one_svg(
    ...
    # =====================================================
    # 4. point sequence 추출 후 SVG path로 복원
    # =====================================================
    gen_point_seq = clean_generated_point_sequence(out_text)

    print("\n[CLEANED POINT SEQUENCE]")
    print(gen_point_seq[:1000])

    gen_path_d = point_sequence_to_svg_path(
        gen_point_seq,
        scale=coord_scale,
    )
